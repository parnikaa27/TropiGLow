...
const Hero: React.FC = () => {
  ...
  return (
    <section id="home" className="relative h-screen overflow-hidden">
      ...
      <div className="absolute inset-0">
        {slides.map((slide, index) => (
          ...
          >
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-r from-slate-900/60 via-blue-900/70 to-slate-800/60"></div>
          </div>
        ))}
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl">
            <h1 className="text-5xl md:text-6xl font-bold text-white mb-6 leading-tight">
              Welcome to <span className="text-blue-200">TropiGlow</span>
            </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
              <div className="flex items-center space-x-3 text-white bg-white bg-opacity-10 backdrop-blur-sm rounded-lg p-4 border border-blue-300">
            ...
            <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6">
              <button 
                ...
                className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-8 py-4 rounded-lg font-semibold text-lg transition-all duration-200 shadow-xl hover:shadow-2xl transform hover:-translate-y-1"
              >
                Explore Products
              </button>
              <button 
                onClick={() => scrollToSection('contact')}
                className="border-2 border-blue-400 text-white hover:bg-blue-500 hover:text-white px-8 py-4 rounded-lg font-semibold text-lg transition-all duration-200 backdrop-blur-sm"
              >
                Contact Us Today
              </button>
            </div>
      ...
      <button
        onClick={prevSlide}
        className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-20 hover:bg-opacity-30 text-white p-3 rounded-full transition-all duration-200 z-20 backdrop-blur-sm border border-blue-300"
      >
        <ChevronLeft className="h-6 w-6" />
      </button>
      <button
        onClick={nextSlide}
        className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-20 hover:bg-opacity-30 text-white p-3 rounded-full transition-all duration-200 z-20 backdrop-blur-sm border border-blue-300"
      >
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all duration-200 ${
              index === currentSlide ? 'bg-blue-300 shadow-lg' : 'bg-white bg-opacity-50'
            }`}
          />
        ))}
      </div>
